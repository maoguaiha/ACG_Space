...
4. **P2优先级**：完善DTO参数校验注解
5. **P2优先级**：补充单元测试用例
6. **Agent 应用（新增）**：设计已锁定（Python agent 嵌入 + Bangumi 只读工具）。Phase 0~4 已落地（骨架 + 语料/导出 + RAG 流式 + Bangumi 工具 + Java 门面 SSE 代理/持久化/限流熔断）；下一步 **Phase 5**（Nuxt 用户端 UI：聊天页 + 流式渲染 + 三主题适配 + 历史会话列表）
