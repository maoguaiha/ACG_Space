...
- [2026-07-31] 错误：python-agent streaming 阶段把 `<longcat_tool_call>` XML 工具调用直接漏到前端（用户截图）| 原因：commit 1c162a9 把拦截从"截断 stream + yield error"放宽成"continue 静默跳过"，且单 token 子串检测被跨 token 切碎绕过（LongCat 流式按 token 切，可能切成 `["<longcat_", "tool_", "call>"]`）| 防范：1) LLM 流式拦截器必须用累积 buffer + hold-back（找到 buffer 末尾最后一个 `<` 暂扣），不是单 token 子串；2) 命中即 `break + yield error`，不是 `continue`；3) 跨供应商（LongCat/Agnes）需检测两种开标签，Agnes AI 的 `<tool_call>` 含 U+200B 零宽字符，**用 `chr(0x200B)` 显式构造字符串**写文件
- [2026-07-31] 错误：python-agent 部署到 Railway 后聊天"思考大几十秒"，本地跑却正常 | 原因（三个叠加）：①**FastEmbed 首次使用在线下载 BAAI/bge-small-zh-v1.5（~33MB）**，Railway 容器无缓存且 huggingface.co/hf-mirror.com 从本机/新加坡实测 HTTP 000 不可达 → 首次对话懒加载 build_corpus 卡死几十秒，/health 报 corpus_ready=false → Railway 反复重启；②每次对话都先跑一轮 non-streaming chat_completion 探测 tool_calls，LongCat 不支持标准 tool_calls，探测=完整生成一遍（5-15s），简单问题也白等；③history 16 条（8 轮）+ RAG 5 段把 prompt 撑到 ~12K tokens，LongCat 长 prompt + tools 下 TTFT 20-40s | 防范：①embedding 用已验证的**通义 text-embedding-v3 API**（dashscope，分批 ≤8/批），不要用 FastEmbed 本地 ONNX（HF 下载不可达）；②**意图预判** `_needs_tools()` 关键词匹配（番剧/推荐/新番/评分/类似/搜/《等），非番剧问题跳过探测直接一次 streaming 不传 tools；③history 16→4、RAG top_k 5→3；④corpus 不可用降级纯 LLM 问答（context 置空）不中断 | 修复 commit：ae529b7（5/5 单测通过）
