-- ============================================================
-- Agent 应用（用户端 AI 助手）会话 / 消息 / 分组表 — V2.4 千问式侧边栏
-- 风格与项目现有表一致：id BIGINT 雪花主键、审计字段、del_flag 逻辑删除
-- 执行方式：
--   1) 全新库：可直接执行（CREATE TABLE IF NOT EXISTS 幂等）
--   2) 存量库：手动执行 agent_groups_v2_4.sql 升级（添加 pinned/group_id 列 + 索引 + 分组表）
...
CREATE TABLE IF NOT EXISTS `agent_conversation` (
  ...
  `group_id`   BIGINT       DEFAULT NULL            COMMENT '所属分组ID(NULL=未分组/最近对话)',
  ...
  KEY `idx_user_pinned_update` (`user_id`, `pinned` DESC, `update_time` DESC),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='AI助手会话分组表';
