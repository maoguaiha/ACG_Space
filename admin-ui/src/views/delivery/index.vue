    ...
    <el-card shadow="never">
      <template #header>
        <div class="card-header">
          <span class="font-bold">O2O 物流调度台</span>
          <div class="header-tips">
            <el-tag type="success" effect="dark">{{ pendingCount }} 待发货</el-tag>
          </div>
        </div>
      </template>
        ...
        <el-table-column label="用户" width="120" align="center">
          <template #default="scope">
            <div class="flex items-center gap-2">
              ...
              <span class="text-sm">{{ scope.row.userName }}</span>
            </div>
          </template>
        </el-table-column>
        <el-table-column label="核销资产" min-width="180" show-overflow-tooltip>
          <template #default="scope">
            <div class="flex items-center gap-2">
              <el-image
        ...
        <el-table-column label="收货信息" min-width="200" show-overflow-tooltip>
          <template #default="scope">
            <div class="text-sm">
              ...
              <p class="text-gray-500">{{ scope.row.address }}</p>
            </div>
          </template>
        </el-table-column>
    ...
    >
      <div v-if="currentDelivery">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="用户">{{ currentDelivery.userName }}</el-descriptions-item>
          <el-descriptions-item label="手机号">{{ currentDelivery.phone }}</el-descriptions-item>
          <el-descriptions-item label="收货人">{{ currentDelivery.receiver }}</el-descriptions-item>
          <el-descriptions-item label="资产名称">
            <div class="flex items-center gap-2">
              <el-image :src="currentDelivery.itemImage" style="width: 40px; height: 40px; border-radius: 4px;" />
              <span>{{ currentDelivery.itemName }}</span>
            </div>
          </el-descriptions-item>
          <el-descriptions-item label="收货地址" :span="2">{{ currentDelivery.address }}</el-descriptions-item>
          <el-descriptions-item label="申请时间">{{ currentDelivery.createTime }}</el-descriptions-item>
...
function handleDetail(row: Delivery) {
  currentDelivery.value = row
