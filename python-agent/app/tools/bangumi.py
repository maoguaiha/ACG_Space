...
def search_bangumi(keyword: str, limit: int = 5) -> dict:
    ...
    # 之前用"前 2 字"匹配太宽松：搜"魔女之旅"会带回"喀耳刻的魔女"、"小魔女DoReMi"。
    # 修复：
    #   - 若关键词在 _SYNONYM_MAP 中（类型词，如"悬疑"）→ **不做标题过滤**——
    #     类型词几乎不出现在番名里（"虚构推理"不含"悬疑"），靠日本过滤 + limit 截断。
    #   - 否则（具体番名，如"魔女之旅"）→ 只保留 title/name_cn/name **完整包含**关键词的结果。
    primary_kw = keyword.strip().lower()
    is_type_word = keyword.strip() in _SYNONYM_MAP
    if not is_type_word:
        filtered = []
        for it in pool:
